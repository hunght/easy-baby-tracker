...
export type GrowthRecordPayload = Omit<GrowthRecordInsert, 'id' | 'recordedAt' | 'babyId'> & {
  babyId?: number;
};
...
export async function getGrowthRecordById(
  ...
  const result = await db
    ...
    .from(schema.growthRecords)
    .where(
      and(eq(schema.growthRecords.id, recordId), eq(schema.growthRecords.babyId, resolvedBabyId))
    )
    .limit(1);
