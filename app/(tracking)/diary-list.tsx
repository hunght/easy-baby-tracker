...
import type { DiaryEntryDoc } from "@/database/diary";
import { useLocalization } from "@/localization/LocalizationProvider";

type GroupedEntry = {
  label: string;
  entries: (DiaryEntryDoc & { date: Date })[];
};
...
export default function DiaryListScreen() {
  ...
  const isLoading = profile === undefined || (profile && entries === undefined);
  ...
  const groupedEntries = useMemo<GroupedEntry[]>(() => {
    ...
    const groups = new Map<string, (DiaryEntryDoc & { date: Date })[]>();
          ...
          {groupedEntries.map((group) => (
            <View key={group.label} className="mb-6">
              ...
              {group.entries.map((entry) => (
                <Pressable
                  key={entry._id}
                  onPress={() => handleOpenEntry(entry._id)}
