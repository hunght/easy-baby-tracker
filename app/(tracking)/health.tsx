...
import type { HealthRecordType, MedicineType } from "@/database/health";
...
const healthTabs: HealthTabOption[] = [
  ...
  { key: "symptom", labelKey: "health.tabs.symptoms", icon: "magnify" },
];
...
export default function HealthScreen() {
  ...
  const existingData = useQuery(
    api.healthRecords.getById,
    recordId ? { healthRecordId: recordId as any } : "skip"
  );

  // Convex mutations
  const createRecord = useMutation(api.healthRecords.create);
  const updateRecord = useMutation(api.healthRecords.update);

  // Populate state when data is loaded
  useEffect(() => {
    if (existingData) {
      setActiveTab(existingData.type as HealthRecordType);
      ...
      if (existingData.type === "temperature") {
        ...
        setMedicineType((existingData.medicineType as MedicineType) ?? "medication");
        setMedication(existingData.medication ?? "");
      } else if (existingData.type === "symptom") {
  ...
  const handleTabChange = (value: string | undefined) => {
    if (
      value &&
      (value === "temperature" || value === "medicine" || value === "symptom")
    ) {
  ...
  const handleSave = async () => {
    ...
    if (activeTab === "symptom" && !symptoms.trim()) return;
    ...
    try {
      ...
      if (isEditing && recordId) {
        await updateRecord({ healthRecordId: recordId as any, ...payload });
      } else {
  ...
  const canSave = () => {
    ...
    if (activeTab === "symptom") return symptoms.trim().length > 0;
    return false;
  };
  ...
  const getTemperatureStatus = () => {
    ...
    if (temperature <= 37.5)
      return { color: "#22C55E", label: t("health.tempNormal") };
    if (temperature <= 38.5)
      return { color: "#F59E0B", label: t("health.tempMild") };
              ...
              {healthTabs.map((tab, index) => (
                ...
                >
                  <MaterialCommunityIcons
                    ...
                    size={20}
                    color={
                      activeTab === tab.key ? brandColors.colors.white : "#666666"
                    }
                  />
