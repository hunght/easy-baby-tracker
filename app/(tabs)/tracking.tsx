...
export default function TrackingScreen() {
  ...
  const feedingsToday =
    useQuery(
      api.feedings.list,
      profile?._id
        ? { babyId: profile._id, startDate: startOfToday, endDate: endOfToday }
        : "skip"
    ) ?? [];
  ...
  useEffect(() => {
    if (profile !== undefined && babyProfiles !== undefined) {
      if (profile === null) {
  ...
  const tileSublabels = useMemo(() => {
    ...
    const totalPumpedMlYesterday = sum(
      pumpingsYesterday.map((p) => p.amountMl ?? 0)
    );
    const lastPumping = maxTimestamp(pumpingsToday.map((p) => p.startTime));
    const lastPumpingAgo =
      lastPumping != null ? formatTimeAgo(nowSeconds - lastPumping) : null;
    const totalSleepSeconds = sum(
      sleepSessionsToday.map((s) => s.duration ?? 0)
    );
    const totalSleepSecondsYesterday = sum(
      sleepSessionsYesterday.map((s) => s.duration ?? 0)
    );
    const longestSleepSeconds = maxTimestamp(
      sleepSessionsToday.map((s) => s.duration ?? 0)
    );
    ...
    const wetCountYesterday = diapersYesterday.filter(
      (d) => d.kind === "wet"
    ).length;
    const dirtyCountYesterday = diapersYesterday.filter(
      (d) => d.kind === "soiled"
    ).length;
    const mixedCountYesterday = diapersYesterday.filter(
      (d) => d.kind === "mixed"
    ).length;
    ...
    const completedHabitIds = new Set(
      todayHabitLogs.map((log) => log.babyHabitId)
    );
    const completedCount = babyHabits.filter((h) =>
      completedHabitIds.has(h._id)
    ).length;
