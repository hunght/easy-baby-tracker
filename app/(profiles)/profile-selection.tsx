...
export default function ProfileSelectionScreen() {
  ...
  const handleSelectProfile = async (babyId: string) => {
    await setActiveBabyMutation.mutateAsync(babyId as any);
        ...
        <View className="gap-4 pb-4">
          {profiles.map((profile) => {
            ...
            return (
              <Pressable
                key={profile._id}
                onPress={() => handleSelectProfile(profile._id)}
              ...
              >
                ...
                {profile.concerns && profile.concerns.length > 0 && (
                  <Text className="mt-1 text-sm text-neutral-500">
                    {t("profileSelection.focusPrefix")}:{" "}
                    {profile.concerns.slice(0, 2).join(", ")}
                  </Text>
                )}
              </Pressable>
            );
